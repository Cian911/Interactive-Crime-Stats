...
var theme_dir = './assets/';
var manifest = require('asset-builder')(theme_dir+'manifest.json');
...
// and project JS.
// ['eslint'] - eslinting disabled because too many problems at the moment
gulp.task('scripts', function() {
  ...
  manifest.forEachDependency('js', function(dep) {
    ...
    deppipe.pipe(streamify(concat(dep.name)))
      ...
      .pipe(streamify(debug({title: 'merged'})))
      // .pipe(browserSync.stream, {match: '**/*.{js,css}'})
      .pipe(rev.manifest(revManifest, {
        base: path.dist,
        merge: true
      }))
      .pipe(streamify(debug({title: 'manifest'})))
      .pipe(gulp.dest(path.dist + 'scripts'))
    merged.add(deppipe);
  });
  // console.log(merged);
...
var svgSpriteConfig = {
  ...
  shape: {
    ...
    meta: path.source + 'assets/img/svg-icons/icons.yaml'
  }
};

gulp.task('icons', function () {
  // return gulp.src(path.source + 'assets/img/svg-icons/**/*.svg')
